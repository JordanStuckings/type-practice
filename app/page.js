...
const sanitizeDictionaryWord = (entry) => {
...
const buildLessonWords = (letters, seed, dictionary) => {
  ...
  const source = dictionary.length > 0 ? dictionary : FALLBACK_WORD_BANK;
  const availableWords = source.filter((word) => wordUsesLetters(word, letters));
  if (availableWords.length === 0) {
    return letters.map((letter) => `${letter}${letter}${letter}`);
  }

  const latestLetter = letters.at(-1);
  const letterFocusedWords = latestLetter
    ? availableWords.filter((word) => word.includes(latestLetter))
    : availableWords;
  let pool = letterFocusedWords;

  if (pool.length === 0) {
    if (latestLetter) {
      const helperWord = `${latestLetter}${letters[0] ?? latestLetter}${latestLetter}`;
      pool = [helperWord];
    } else {
      pool = availableWords;
    }
...
const computeLineBreakIndex = (text) => {
  if (!text) return 0;
  const midpoint = Math.floor(text.length / 2);
  const forward = text.indexOf(" ", midpoint);
  const backward = text.lastIndexOf(" ", midpoint);
  ...
  return forward - midpoint <= midpoint - backward ? forward + 1 : backward + 1;
};
...
export default function Home() {
  ...
  const lineBreakIndex = useMemo(() => computeLineBreakIndex(lessonText), [lessonText]);
    ...
    const loadDictionary = async () => {
      try {
        ...
        for (let i = 0; i < lines.length; i += 1) {
          const sanitized = sanitizeDictionaryWord(lines[i]);
...
function LessonSurface({ highlightedPrompt, onClick, lineBreakIndex }) {
  const breakPosition =
    lineBreakIndex && lineBreakIndex < highlightedPrompt.length
      ? lineBreakIndex
  ...
  return (
    ...
    >
      <div className="flex w-full max-w-6xl flex-col items-center gap-8">
        <p className="flex w-full flex-wrap justify-center gap-4 text-center font-mono text-4xl tracking-tight text-white sm:text-5xl">
          {firstLine}
        </p>
        {secondLine.length > 0 ? (
          <p className="flex w-full flex-wrap justify-center gap-4 text-center font-mono text-4xl tracking-tight text-white sm:text-5xl">
